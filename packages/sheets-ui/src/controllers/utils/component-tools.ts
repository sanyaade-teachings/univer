...
export function getCoordByOffset(
    ...
    const scrollXY = scene.getViewportScrollXYByCursorViewport(relativeCoords, viewport);
...
export function getTransformCoord(evtOffsetX: number, evtOffsetY: number, scene: Scene, skeleton: SpreadsheetSkeleton) {
    ...
    const scrollXY = scene.getViewportScrollXYByCursorViewport(relativeCoords, viewMain);
    const { scaleX, scaleY } = scene.getAncestorScale();
